...
if (themeToggle) {
  ...
  try { manuallySelected = ["light", "dark"].includes(localStorage.getItem("site-theme")); } catch { /* Storage may be unavailable. */ }
  ...
  themeToggle.addEventListener("click", () => {
    ...
    try { localStorage.setItem("site-theme", next); } catch { /* Keep the current-page choice. */ }
    updateThemeToggle();
  });
...
if (readingArticle) {
  ...
  progress.innerHTML = '<span class="reading-progress-track" role="slider" tabindex="0" aria-label="Article reading progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><span class="reading-progress-fill"></span><span class="reading-progress-mark"></span></span><span class="reading-progress-label" aria-hidden="true">0%</span>';
  ...
  const revealActiveHeading = () => {
    ...
    if (linkBounds.top < navBounds.top) outlineNav.scrollTop += linkBounds.top - navBounds.top;
    else if (linkBounds.bottom > navBounds.bottom) outlineNav.scrollTop += linkBounds.bottom - navBounds.bottom;
  };
  if (headings.length >= 3) {
    ...
    outlineLinks = headings.map((heading, index) => {
      ...
      if (heading.tagName === "H3") item.className = "reading-outline-subsection";
    ...
    wideOutline.addEventListener("change", () => setOutlineOpen(wideOutline.matches));
    document.addEventListener("click", (event) => {
      if (outline.classList.contains("open") && !outline.contains(event.target)) setOutlineOpen(false);
    });
    ...
    if (window.location.hash) {
      try {
        ...
        if (headings.includes(target)) requestAnimationFrame(() => target.scrollIntoView());
  ...
  const jumpToFraction = (fraction) => {
    ...
    if (end <= start) return;
    window.scrollTo(0, start + Math.min(1, Math.max(0, fraction)) * (end - start));
  };
  ...
  track.addEventListener("keydown", (event) => {
    const steps = { ArrowDown: .05, ArrowRight: .05, ArrowUp: -.05, ArrowLeft: -.05, PageDown: .2, PageUp: -.2 };
    if (event.key === "Home" || event.key === "End" || event.key in steps) {
      event.preventDefault();
      const { start, end } = readingRange();
      const current = end > start ? Math.min(1, Math.max(0, (window.scrollY - start) / (end - start))) : 0;
      jumpToFraction(event.key === "Home" ? 0 : event.key === "End" ? 1 : current + steps[event.key]);
    }
  });
  ...
  const updateProgress = () => {
    ...
    if (!progress.hidden) {
      const fraction = Math.min(1, Math.max(0, (window.scrollY - start) / (end - start)));
      const percent = Math.round(fraction * 100);
      progress.style.setProperty("--reading-progress", String(fraction));
      track.setAttribute("aria-valuenow", String(percent));
      track.setAttribute("aria-orientation", window.matchMedia("(max-width: 900px)").matches ? "horizontal" : "vertical");
      label.textContent = `${percent}%`;
    }
    if (outlineLinks.length) {
      const active = headings.findLastIndex((heading) => heading.getBoundingClientRect().top <= window.innerHeight * .35);
  ...
  if ("ResizeObserver" in window) new ResizeObserver(scheduleProgressUpdate).observe(readingArticle);
...
if (archiveQuery) {
  ...
  archiveQuery.addEventListener("input", () => {
    ...
    const matches = noteItems.filter((note) => note.dataset.search.includes(query));
    results.replaceChildren(...matches.map((note) => {
      const item = document.createElement("li");
...
async function startTerminal() {
  ...
  const openTerminal = () => {
    if (!terminal.hidden) { focusCommandInput(); return; }
    returnFocus = document.activeElement instanceof HTMLElement ? document.activeElement : terminalToggle;
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !terminal.hidden) { event.preventDefault(); closeTerminal(); return; }
    if (event.key === "`" && !event.repeat && !/INPUT|TEXTAREA/.test(document.activeElement?.tagName ?? "")) {
  ...
  const commands = ["help", "pwd", "ls", "cd", "tree", "open", "cat", "random", "whoami", "home", "history", "clear", "coffee", "heatmap", "exit"];
  const coffeeJokes = [
    "程序员为什么喜欢深色模式？因为亮光会引来 bug。",
    "我问程序为什么不工作。它说：这不是 bug，是未记录的特性。",
    "程序员去买面包。家人说：买两个，如果有鸡蛋就买十个。于是他带回十个面包。",
    "要理解递归，先要理解递归。",
    "今天的代码很稳定：它每次都以同一种方式崩溃。",
    "为什么数据库管理员不爱讲笑话？因为总有人要求解释关系。",
  ];
  const coffeeArt = "    ( (\n     ) )\n  ........\n  |      |]\n  \\      /\n   `----'";
  const saved = readTerminalState();
  const commandHistory = Array.isArray(saved.history) ? saved.history.slice(-40) : [];
  const logLines = Array.isArray(saved.lines) ? saved.lines.slice(-60) : [];
  let historyIndex = commandHistory.length;
  let cwd = "/";

  const save = () => { try { sessionStorage.setItem("nomad-terminal", JSON.stringify({ history: commandHistory.slice(-40), lines: logLines.slice(-60) })); } catch { /* session history is optional */ } };
  ...
  const print = (text = "", type = "") => {
    ...
    logLines.push({ text, type }); save();
    terminalOutput.scrollTop = terminalOutput.scrollHeight;
  };
  const showCwd = () => { terminalCwd.textContent = cwd === "/" ? "~" : `~${cwd}`; };
  const resolve = (input = "") => {
  ...
  const canonicalDirectory = (target) => [...directories].find((directory) => directory.toLowerCase() === target.toLowerCase());
  const notePath = (note) => cleanPath(note.path);
  const findNote = (query) => {
    const resolved = resolve(query);
    const lowered = query.toLowerCase();
    return notes.find((note) => notePath(note).toLowerCase() === resolved.toLowerCase())
      ?? notes.find((note) => note.title.toLowerCase() === lowered)
      ?? notes.find((note) => parentPath(notePath(note)) === cwd && note.title.toLowerCase().startsWith(lowered));
  };
  const list = (target) => {
    const directory = canonicalDirectory(resolve(target));
    if (!directory) return null;
    const childDirectories = [...directories].filter((item) => item !== directory && parentPath(item) === directory).map(baseName).sort();
    const childNotes = notes.filter((note) => parentPath(notePath(note)) === directory).map((note) => note.title).sort();
    return [...childDirectories.map((name) => `${name}/`), ...childNotes];
  };
  const pagePath = cleanPath(window.location.pathname);
  const pageNote = notes.find((note) => notePath(note).toLowerCase() === pagePath.toLowerCase());
  cwd = pageNote ? parentPath(notePath(pageNote)) : canonicalDirectory(pagePath) ?? "/";
  ...
  const run = (rawCommand) => {
    ...
    commandHistory.push(raw); historyIndex = commandHistory.length; save();
    const [name = "", ...args] = raw.match(/"[^"]*"|'[^']*'|\S+/g)?.map((part) => part.replace(/^(['"])(.*)\1$/, "$2")) ?? [];
    const argument = args.join(" ");

    switch (name.toLowerCase()) {
      case "help": print("help  pwd  ls [path]  cd <path>  tree [path]  open <note>\ncat <about|now>  random  whoami  home  history  clear  coffee  heatmap  exit"); break;
      ...
      case "tree": {
        ...
        if (!root) { print(`tree: ${argument}: no such directory`, "error"); break; }
        const rows = [root === "/" ? "." : baseName(root)];
        for (const directory of [...directories].filter((item) => item !== root && isWithin(item, root)).sort()) rows.push(`${"  ".repeat(depthFrom(directory, root))}├── ${baseName(directory)}/`);
        for (const note of notes.filter((item) => isWithin(notePath(item), root)).sort((a, b) => notePath(a).localeCompare(notePath(b)))) rows.push(`${"  ".repeat(depthFrom(notePath(note), root))}└── ${note.title}`);
        print(rows.join("\n"));
        break;
      }
      case "open": {
        ...
        else if (directory) window.location.href = directory === "/" ? "/" : `${directory}/`;
        else print(`open: ${argument}: not found`, "error");
        break;
      }
      case "cat": {
        const note = findNote(argument);
        if (!note || !["about", "now"].includes(argument.toLowerCase())) print("cat: available public documents are about and now", "error");
        else print(`${note.title}\n${"-".repeat(Math.min(note.title.length, 24))}\n${note.text || "(empty)"}`);
        break;
      }
      case "random": {
        if (!notes.length) print("random: no published notes yet", "error");
        else window.location.href = notes[Math.floor(Math.random() * notes.length)].url;
        break;
      }
      case "whoami": print(`${garden.author}\n${garden.tagline}\n${garden.subtitle}`); break;
      case "home": window.location.href = "/"; break;
      case "history": print(commandHistory.map((command, index) => `${String(index + 1).padStart(2, " ")}  ${command}`).join("\n")); break;
      case "clear": terminalOutput.replaceChildren(); logLines.length = 0; save(); break;
      case "coffee": print(`${coffeeArt}\n\n${coffeeJokes[Math.floor(Math.random() * coffeeJokes.length)]}`, "hint"); break;
      case "heatmap": print(renderNoteHeatmap(notes), "hint"); break;
      case "exit": closeTerminal(); break;
      default: print(`${name}: command not found. Try help.`, "error");
    }
  };

  terminalForm.addEventListener("submit", (event) => { event.preventDefault(); run(terminalInput.value); terminalInput.value = ""; });
  terminalInput.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Tab") {
      event.preventDefault();
      const parts = terminalInput.value.split(/\s+/); const fragment = parts.at(-1)?.toLowerCase() ?? "";
      const candidates = parts.length === 1 ? commands : [...directories].map(baseName).concat(notes.map((note) => note.title));
      const matches = candidates.filter((candidate) => candidate.toLowerCase().startsWith(fragment));
      if (matches.length === 1) { parts[parts.length - 1] = matches[0]; terminalInput.value = parts.join(" "); terminalInput.setSelectionRange(terminalInput.value.length, terminalInput.value.length); }
...
function cleanPath(value) { const cleaned = `/${String(value ?? "").split("/").filter(Boolean).join("/")}`; return cleaned === "/" ? cleaned : cleaned.replace(/\/$/, ""); }
function parentPath(value) { const parts = cleanPath(value).split("/").filter(Boolean); parts.pop(); return parts.length ? `/${parts.join("/")}` : "/"; }
function baseName(value) { return cleanPath(value).split("/").filter(Boolean).at(-1) ?? "/"; }
function isWithin(value, root) { return root === "/" ? value !== "/" : value.startsWith(`${root}/`); }
function depthFrom(value, root) { return cleanPath(value).split("/").filter(Boolean).length - (root === "/" ? 0 : cleanPath(root).split("/").filter(Boolean).length); }



function readTerminalState() {
  try { return JSON.parse(sessionStorage.getItem("nomad-terminal") ?? "{}"); }
  catch { return {}; }
}
